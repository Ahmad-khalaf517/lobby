...
- **Supabase Auth is the identity system.** Angular's singleton public-key Supabase client owns registered and anonymous browser sessions, persistence, and refresh. Angular sends the current access token to NestJS as a Bearer token; NestJS cookies remain a supported API/Postman flow.
