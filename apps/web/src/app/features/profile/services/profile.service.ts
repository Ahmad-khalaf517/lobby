...
import { replaceEmojiShortcodes } from '../../../shared/utils/emoji-shortcodes.util';
...
export class ProfileService {
  ...
  async updateProfile(userId: string, changes: UpdateUserProfileRequest): Promise<UserProfile> {
    const body: UpdateUserProfileRequest = {
      ...changes,
      bio: changes.bio != null ? replaceEmojiShortcodes(changes.bio) : changes.bio,
    };
    const response = await firstValueFrom(
      this.http.patch<unknown>(`${this.apiUrl}/users/${userId}/profile`, body),
    );
    return UserProfileSchema.parse(response);
  }
