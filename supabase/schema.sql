...
create table if not exists public.channels (
  ...
  id          text primary key,
  -- null means a standalone guest channel (no server). Set only for
...
comment on table  public.channels                     is 'A channel inside a server, or a standalone guest channel when server_id is null.';
...
comment on column public.channels.server_id          is 'Parent server that owns this channel. Null for guest channels created without a server.';
comment on column public.channels.name               is 'Channel display name, unique within a server.';
comment on column public.channels.livekit_room_name  is 'Stable LiveKit room identifier for voice call join.';
comment on column public.channels.max_participants   is 'Soft channel participant cap enforced by the backend token endpoint.';
comment on column public.channels.created_at         is 'Insert time for the channel row.';
