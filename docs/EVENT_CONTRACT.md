...
|---|---|---|---|
| `POST /channels` | `{ name? }` | `{ id, name, createdAt }` | `CreateChannelRequestSchema` / `CreateChannelResponseSchema` |
| `GET /channels/:id` | — | `{ id, name, createdAt }` | `ChannelSchema` |
...
| `chatMessage` | `{ name, text, time }` | `ChatMessageBroadcastSchema` |
